...
**更新目标**: 向 Label Studio 企业版看齐，实现完整的企业级功能
**当前状态**: 需要更新以支持新的多租户隔离和企业级功能
...
### Phase 1: TCB 全栈 Docker 镜像更新（第1周）

- [x] 1. 企业级全栈镜像重构 🆕 **新增需求**
  - [x] 1.1 合并 FastAPI + Label Studio + PostgreSQL + Redis 到单镜像
    - 更新 Dockerfile 支持多服务集成
    - 优化启动顺序和依赖管理
    - 确保服务间通信和数据共享
    - 实现健康检查和故障恢复
    - _需求 1: TCB 全栈 Docker 镜像构建_

  - [x] 1.2 TCB cloudbaserc.json 配置更新
    - 配置 Serverless 云托管参数
    - 设置自动扩缩容规则（1-10实例）
    - 配置环境变量和密钥管理
    - 设置资源限制和成本控制
...
### Phase 2: 多租户隔离基础（第1-2周）

- [x] 2. 数据库多租户改造 🆕 **新增需求**
  - [x] 2.1 数据表 tenant_id 字段添加
  ...
  - [x] 2.3 Label Studio 多项目隔离
    - 配置每个租户独立的 Label Studio 项目
    - 实现项目级别的数据隔离
    - 配置租户级别的用户权限
    - 实现项目间的安全隔离
    - _需求 7: 安全和合规_
