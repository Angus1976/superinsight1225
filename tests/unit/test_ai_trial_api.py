...
def test_db():
    ...
    # Patch UUID columns for SQLite (AuditLogModel only). Keep originals so we
    # restore the exact TypeEngine instances — global model mutations must not
    # leak to other test modules (e.g. annotation_task_api).
    uuid_original_types = {}
    for col in AuditLogModel.__table__.columns:
        if isinstance(col.type, PGUUID):
            uuid_original_types[col] = col.type
            col.type = SQLiteUUID()
    ...
    try:
        ...
        for col, original_type in uuid_original_types.items():
...
def _create_trial(client, **overrides):
    ...
    return client.post("/api/ai-trials", json=payload)
...
def _create_and_execute_trial(client, **overrides):
    ...
    exec_resp = client.post(f"/api/ai-trials/{trial_id}/execute", json={"source_data": source_data})
...
def test_execute_trial_already_completed(client):
    ...
    response = client.post(
        f"/api/ai-trials/{trial_id}/execute",
        json={"source_data": [{"text": "test"}]},
    )
...
def test_get_results_success(client):
    ...
    trial_id, _ = _create_and_execute_trial(client)
    response = client.get(f"/api/ai-trials/{trial_id}/results")
...
def test_get_results_not_found(client):
    ...
    response = client.get(f"/api/ai-trials/{uuid4()}/results")
    assert response.status_code == 404
...
def test_compare_trials_success(client):
    ...
    response = client.post("/api/ai-trials/compare", json={
        "trial_ids": [tid1, tid2],
    })
    assert response.status_code == 200
    data = response.json()
...
def test_cancel_created_trial(client):
    ...
    response = client.post(f"/api/ai-trials/{trial_id}/cancel")
    assert response.status_code == 200
...
def test_cancel_not_found(client):
    ...
    response = client.post(f"/api/ai-trials/{uuid4()}/cancel")
    assert response.status_code == 404
...
def test_cancel_already_cancelled(client):
    ...
    resp = _create_trial(client)
    trial_id = resp.json()["id"]
    client.post(f"/api/ai-trials/{trial_id}/cancel")
    response = client.post(f"/api/ai-trials/{trial_id}/cancel")
    assert response.status_code == 400
...
def test_list_trials_empty(client):
    ...
    response = client.get("/api/ai-trials")
    assert response.status_code == 200
    data = response.json()
    assert data["items"] == []
    assert data["total"] == 0
...
def test_list_trials_pagination(client):
    ...
    response = client.get("/api/ai-trials?page=1&page_size=2")
    data = response.json()
    assert data["total"] == 5
    ...
    response2 = client.get("/api/ai-trials?page=3&page_size=2")
...
def test_list_trials_filter_by_status(client):
    ...
    response = client.get("/api/ai-trials?status_filter=created")
    data = response.json()
    assert data["total"] == 1
    assert data["items"][0]["status"] == "created"

    response2 = client.get("/api/ai-trials?status_filter=completed")
    data2 = response2.json()
...
def test_list_trials_filter_by_data_stage(client):
    ...
    response = client.get("/api/ai-trials?data_stage=sample_library")
    data = response.json()
    assert data["total"] == 1
...
def test_list_trials_filter_by_model_name(client):
    ...
    response = client.get("/api/ai-trials?model_name=model-a")
    data = response.json()
    assert data["total"] == 1
