...
async function checkI18nConsoleWarnings() {
  ...
  const fs = require('fs');
  const reportPath = 'I18N_CONSOLE_WARNINGS_REPORT.md';
...
async function testPageWithLanguage(page, path, language, pageResult, consoleMessages) {
  const startTime = new Date().toISOString();
  if (!pageResult.startTime) {
    pageResult.startTime = startTime;
  }
  
  const url = `http://localhost:5174${path}?lng=${language}`;
