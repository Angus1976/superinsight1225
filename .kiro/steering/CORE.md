...
**进度信息动态管理**：
- 进度信息存储在 `.kiro/project-progress.json` 文件中
- AI 会在每月初主动询问是否需要更新进度
- 用户可随时要求更新进度
- 完成重要里程碑时应更新进度

**不同阶段的工作策略**：
- **0-30% (项目初期)**：可大胆尝试、架构设计、快速迭代
- **30-60% (项目中期)**：功能开发、局部重构、架构微调
- **60-85% (接近交付)**：扩展功能、修复 bug、优化代码（当前阶段）
- **85-100% (最终阶段)**：只修复关键 bug、完善文档、微调性能
...
**接近交付阶段的特殊要求**：
- **先分析，后动手**：动手前必须先分析已完成的代码和整体架构
- **理解现有实现**：充分理解历史代码的设计意图和实现方式
- **复用优先**：优先复用现有组件、模式、工具，而非重新实现
- **架构一致性**：新增代码必须符合项目整体架构风格
- **最小改动原则**：能改 1 个文件不改 2 个，能改 10 行不改 20 行

**例外处理**：
- 如必须突破当前阶段规则（如大面积重构），必须征得用户同意
- 说明原因、风险、收益，等待明确批准后再执行
